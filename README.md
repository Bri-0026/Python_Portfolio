# Python_Portfolio
This is the portfolio of python code I learned in Computational Biology.
...
```
    ...
    <ipython-input-1-af8f97c9a1ff> in <module>
...
```python
names = ['Curie', 'Darwing', 'Turing'] #typo on Darwin's name
...
```python
binomial_name = "Drosophila melanogaster" #it's a fruit fly
...
```python
#Prefix to patient id
patient_id = 'inflam_' + patient_id
print(patient_id)
#just patient_id runs correctly for me
```
...
#half of 60-1, half of 40-1
#but actually row 30, column 20
```
...
```python
print(data[0:4, 0:10])
#first 4 patients, first 10 columns
```
...
```
    ...
    <ipython-input-1-baa02f15189e> in normalize_rectangle(rect)
...
```
    ...
    <ipython-input-10-80290759369d> in <module>
    ----> 1 test_range_overlap()
    

    <ipython-input-9-2c8841db23c8> in test_range_overlap()
...
```
    ...
    <ipython-input-12-bf2ffc093826> in <module>
          1 # a possible pre-condition:
    ----> 2 assert len(input_list) > 0, 'List length must be non-zero'
          3 # a possible post-condition:
          4 assert numpy.amin(input_list) <= average <= numpy.amax(input_list)
          5 'Average should be between min and max of input values (inclusive)'
...
```
    ...
    <ipython-input-14-8b7c7a2077ba> in <module>
          8     return total
          9 get_total_cars([1, 2, 3, 4])
    ---> 10 get_total_cars(['a', 'b', 'c'])
    

    <ipython-input-14-8b7c7a2077ba> in get_total_cars(values)
          2     assert len(values) > 0
          3     for element in values:
    ----> 4         assert int(element)
          5     values = [int(element) for element in values]
          6     total = sum(values)
